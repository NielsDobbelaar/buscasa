<template>
    <dialog :open="true" class="singleHouseWrapper">
        ...
        <section class="singleHouse__container">
            ...
            <section class="singleHouse__actions">
                ...
                <section class="singleHouse__houseActions">
                    <button class="singleHouse__actionButton secondary">
                        Info woningtype
                    </button>
                    <button class="singleHouse__actionButton primary">
                        Inschrijven
                    </button>
                </section>
            </section>
...
watch(() => compareIsChecked.value, (value) => {
    if (value === true) {
        housesStore.addCompareNumber(props.houseId)
    } else {
        housesStore.removeCompareNumber(props.houseId)
    }
})
...
// addeventlister if click outside of singleHouse__container then closeOverlay
...
watch(() => props.isSingleHouseOverlayOpen, (value) => {
    if (value === false) {
...
const houseInfo = computed(() => {
    let house = housesStore.getHouseById(props.houseId)

    let houseProperties = ["house_type", "living_surface", "bedrooms", "plot_surface", "room_count", "parking_count", "parking_value"]
